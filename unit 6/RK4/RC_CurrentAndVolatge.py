...
#To Plot Current in RC circuit and potential ODE with DC source by RK4 Method, Exact solution, Inbuilt solver.
...
for i in range(len(time_array)-1):      # updating the array of zeros with help of euler's method and solution equation
    ...
    k2=h*RC.current(yPointsCurrent[i]+(k1/2),time_array[i]+(h/2))
    k3=h*RC.current(yPointsCurrent[i]+(k2/2),time_array[i]+(h/2))
    k4=h*RC.current(yPointsCurrent[i]+(k3),time_array[i]+(h))
...
Vr0=V
yPointsVr=np.zeros(len(time_array))       #initializing Y-coordinates as array of zeros of lenght time array(RK4 method )
...
for i in range(len(time_array)-1):      # updating the array of zeros with help of euler's method and solution equation
    ...
    k2=h*RC.Vr(yPointsVr[i]+(k1/2),time_array[i]+(h/2))
    k3=h*RC.Vr(yPointsVr[i]+(k2/2),time_array[i]+(h/2))
    k4=h*RC.Vr(yPointsVr[i]+(k3),time_array[i]+(h))
    delY=(1/6)*(k1+(2*k2)+(2*k3)+k4)
...
Vc0=0
yPointsVc=np.zeros(len(time_array))       #initializing Y-coordinates as array of zeros of lenght time array(RK4 method )
...
for i in range(len(time_array)-1):      # updating the array of zeros with help of euler's method and solution equation
    ...
    k2=h*RC.Vc(yPointsVc[i]+(k1/2),time_array[i]+(h/2))
    k3=h*RC.Vc(yPointsVc[i]+(k2/2),time_array[i]+(h/2))
    k4=h*RC.Vc(yPointsVc[i]+(k3),time_array[i]+(h))
    delY=(1/6)*(k1+(2*k2)+(2*k3)+k4)
...
plt.xlabel('Time(s)')
plt.ylabel('(volts)')
plt.title("Vr and Vc v/s time RK4s ")
plt.grid('true')
plt.legend()
plt.subplot(3,2,4)
