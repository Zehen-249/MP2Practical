...
# To Plot Newton's cooling law ODE by RK4 method, Exact solution & Inbuilt solver. 
...
print("\n\n\tMehendi Hasan\n\n\t2230248\n\n")
print("Newton's Law of Cooling\n\nTemperature is in Degree Celsius and time is in seconds\n\n")
...
for i in range(len(t_values)-1):
    ...
    k2=h*f(T_values[i]+(k1/2),t_values[i]+(h/2))
    k3=h*f(T_values[i]+(k2/2),t_values[i]+(h/2))
    k4=h*f(T_values[i]+(k3),t_values[i]+(h))
...
#ploting 
plt.subplot(3,1,1)
plt.plot(t_values,T_values,label="Temperature",color="red")
plt.grid()
...
plt.legend()
plt.subplot(3,1,2)
plt.plot(t_values,T_Exact,label='Temperature',color="blue")
plt.grid()
...
plt.legend()
plt.subplot(3,1,3)
plt.plot(t_values,T_odeint,label='Temperature',color="orange")
plt.grid()
