...
#To Plot Current in RC circuit and potential ODE with DC source by Modified Euler Method, Exact solution, Inbuilt solver.
...
for i in range(len(time_array)-1):      # updating the array of zeros with help of euler's method and solution equation
    ...
    yPointsCurrent[i+1] = yPointsCurrent[i] +(h/2)*(RC.current(yPointsCurrent[i],time_array[i])+RC.current(eulersMethod(time_array[i],yPointsCurrent[i],RC.current),time_array[i+1]))
...
Vr0=V
yPointsVr=np.zeros(len(time_array))       #initializing Y-coordinates as array of zeros of lenght time array(Modified Euler method )
...
for i in range(len(time_array)-1):      # updating the array of zeros with help of euler's method and solution equation
    #Modified Euler's Method
    yPointsVr[i+1] = yPointsVr[i] +(h/2)*(RC.Vr(yPointsVr[i],time_array[i])+RC.Vr(eulersMethod(time_array[i],yPointsVr[i],RC.Vr),time_array[i+1]))
...
Vc0=0
yPointsVc=np.zeros(len(time_array))       #initializing Y-coordinates as array of zeros of lenght time array(Modified Euler method )
...
for i in range(len(time_array)-1):      # updating the array of zeros with help of euler's method and solution equation
    yPointsVc[i+1]=yPointsVc[i]+h*RC.Vc(yPointsVc[i],time_array[i])      #Modified Euler's Method
    yPointsVc[i+1] = yPointsVc[i] +(h/2)*(RC.Vc(yPointsVc[i],time_array[i])+RC.Vc(eulersMethod(time_array[i],yPointsVc[i],RC.Vc),time_array[i+1]))
...
plt.xlabel('Time(s)')
plt.ylabel('(volts)')
plt.title("Vr and Vc v/s time Modified Eulers ")
plt.grid('true')
plt.legend()
plt.subplot(3,2,5)
