...
#To Plot Charging and Discharging of a capacitor in RC circuit ODE with DC source by RK2 Method, Exact solution, Inbuilt solver. 
...
Y_diff_charging=np.zeros(len(t_array))      #Initializing array for values of dependent variable(RK2's method)
...
for i in range(len(t_array)-1):                 #updating values of dependent variable
    ...
    k2=h*diff_equ_charging(Y_diff_charging[i]+k1,t_array[i]+h)
    delY=0.5*(k1+k2)
...
Y_diff_discharging=np.zeros(len(t_array))   #Initializing array for values of dependent variable(RK2's method)
...
for i in range(len(t_array)-1):                          # #updating values of dependent variable
    ...
    k2=h*diff_equ_discharging(Y_diff_discharging[i]+k1,t_array[i]+h)
    delY=0.5*(k1+k2)
...
plt.title("RK2's Solution of Charging")
plt.legend()
plt.subplot(3,2,4)
...
plt.xlabel("Time")
plt.ylabel("Charge at Capacitor")
plt.title("RK2's Solution of Discharging")
plt.legend()
plt.subplot(3,2,3)
